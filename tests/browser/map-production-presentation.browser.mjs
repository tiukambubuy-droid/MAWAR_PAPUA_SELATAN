...
await executeBrowserLifecycle({
 ...
 runBrowser:async({port,profileHandle,serverProcess:server,chromeProcess:chrome,getProcessError,registerBrowserClose})=>{
  ...
  for (const [width, height] of viewports) {
    ...
    await evaluate(`(()=>{const input=document.querySelector('#map-region-query'),set=Object.getOwnPropertyDescriptor(HTMLInputElement.prototype,'value').set;set.call(input,'a');input.dispatchEvent(new Event('input',{bubbles:true}));input.focus()})()`);await waitFor(".map-region-options");
    const searchLayout=await evaluate(`(()=>{const input=document.querySelector('#map-region-query'),field=document.querySelector('.map-region-search-field'),panel=document.querySelector('.map-region-options'),card=document.querySelector('.large-map'),next=document.querySelector('.land-insight'),clear=document.querySelector('.map-region-search-field button'),ir=input.getBoundingClientRect(),fr=field.getBoundingClientRect(),pr=panel.getBoundingClientRect(),cr=card.getBoundingClientRect(),nr=next.getBoundingClientRect(),style=getComputedStyle(input),overlap=!(pr.right<=nr.left||pr.left>=nr.right||pr.bottom<=nr.top||pr.top>=nr.bottom);return{inputFont:parseFloat(style.fontSize),inputHeight:ir.height,paddingLeft:parseFloat(style.paddingLeft),paddingRight:parseFloat(style.paddingRight),clearHeight:clear?.getBoundingClientRect().height??0,panelInside:pr.left>=cr.left-1&&pr.right<=cr.right+1&&pr.right<=innerWidth+1,panelBelowInput:pr.top>=fr.bottom-1,panelOverlapsNext:overlap,pageOverflow:document.documentElement.scrollWidth>document.documentElement.clientWidth+1}})()`);
    ...
    assert.ok(searchLayout.panelInside&&searchLayout.panelBelowInput&&!searchLayout.panelOverlapsNext&&!searchLayout.pageOverflow,`${width}x${height}: search bounds ${JSON.stringify(searchLayout)}`);
