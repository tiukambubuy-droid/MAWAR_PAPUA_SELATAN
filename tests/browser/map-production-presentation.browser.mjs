...
await executeBrowserLifecycle({
 ...
 runBrowser:async({port,profileHandle,serverProcess:server,chromeProcess:chrome,getProcessError,registerBrowserClose})=>{
  ...
  for (const [width, height] of viewports) {
    ...
    if(width>900){assert.equal(navigationMetrics.position,"fixed",`${width}x${height}: sidebar fixed`);assert.ok(Math.abs(navigationMetrics.sidebarTop)<=1&&Math.abs(navigationMetrics.sidebarBottom-height)<=1&&Math.abs(navigationMetrics.sidebarHeight-height)<=1,`${width}x${height}: sidebar memenuhi viewport ${JSON.stringify(navigationMetrics)}`)}
    ...
    if(navigationMetrics.drawer){
      ...
      if(width===768||width===430||width===390){
        ...
        await waitUntil("document.activeElement?.classList.contains('nav-item')","fokus awal drawer untuk tombol X");
  ...
  await send("Emulation.setDeviceMetricsOverride",{width:768,height:1024,deviceScaleFactor:1,mobile:false});await waitUntil("getComputedStyle(document.querySelector('.navigation-trigger')).display!=='none'","transisi desktop ke drawer");
  await evaluate("document.querySelector('.navigation-trigger').click()");await waitUntil("document.querySelector('.sidebar').classList.contains('is-open')","drawer transisi terbuka");assert.equal(await evaluate("document.body.style.overflow"),"hidden");
  await send("Emulation.setDeviceMetricsOverride",{width:1440,height:900,deviceScaleFactor:1,mobile:false});await waitUntil("getComputedStyle(document.querySelector('.navigation-trigger')).display==='none'&&!document.querySelector('.sidebar').classList.contains('is-open')","transisi drawer ke fixed bersih");
  assert.equal(await evaluate("document.body.style.overflow"),"");assert.deepEqual(await evaluate("(()=>{const overlay=document.querySelector('.navigation-overlay');return{display:getComputedStyle(overlay).display,open:overlay.classList.contains('is-open')}})()"),{display:"none",open:false});assert.equal(await evaluate("getComputedStyle(document.querySelector('.sidebar')).position"),"fixed");
  ...
  for(const [baseWidth,baseHeight] of viewports)for(const factor of zoomFactors){
    ...
    await send("Emulation.setDeviceMetricsOverride",{width,height,deviceScaleFactor:1,mobile:false});
    await send("Page.navigate",{url:`${origin}/?season=MT2-2026`});await waitFor(".sidebar nav");
    const drawerExpected=width<=900||height<=720;if(drawerExpected){await evaluate("document.querySelector('.navigation-trigger').click()");await waitUntil("document.querySelector('.sidebar').classList.contains('is-open')",`${context}: drawer terbuka`);await waitUntil("document.activeElement?.classList.contains('nav-item')",`${context}: fokus drawer`)}
