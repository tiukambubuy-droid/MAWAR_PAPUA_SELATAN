...
export function SeasonCalendar({ title, months, active, updatedAt, onSelect }: { title: string; months: MonthObservation[]; active: number; updatedAt: string; onSelect: (index: number) => void }) {
  ...
  return <article className="card season-calendar">
    ...
    <div className="season-calendar-focus"><span>Fokus {selected.label} {selected.year}</span><strong>{selected.focus}</strong><em>Data diperbarui {updatedAt} · Validasi {selected.validation}%</em></div>
  </article>;
}
