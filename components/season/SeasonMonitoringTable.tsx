...
export function SeasonMonitoringTable({ title, rows, entityLabel, onSelect, onDetail }: {
  ...
  return <article className="card season-table-card"><div className="season-section-title">{title}</div>
    ...
    <div className="season-table-footer"><span>Menampilkan {filtered.length ? (page - 1) * 5 + 1 : 0}–{Math.min(page * 5, filtered.length)} dari {filtered.length} data</span>{filtered.length > 0 && <div><button disabled={page === 1} onClick={() => setPage(p => p - 1)}>‹</button>{Array.from({ length: pages }, (_, i) => <button className={page === i + 1 ? "active" : ""} onClick={() => setPage(i + 1)} key={i}>{i + 1}</button>)}<button disabled={page === pages} onClick={() => setPage(p => p + 1)}>›</button></div>}</div>
  </article>;
}
