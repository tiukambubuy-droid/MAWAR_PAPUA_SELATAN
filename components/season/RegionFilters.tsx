...
export function RegionFilters({ districts, villages, district, village, onDistrict, onVillage }: {
  ...
  return <div className="season-filter-group region"><div className="season-filter-title">WILAYAH</div><div className="season-filter-pair three">
    ...
    <label><span>Kampung</span><select aria-label="Kampung" disabled={district === "all"} value={village} onChange={e => onVillage(e.target.value)}><option value="all">Semua Kampung</option>{villages.map(item => <option key={item.id}>{item.name}</option>)}</select></label>
  </div></div>;
}
