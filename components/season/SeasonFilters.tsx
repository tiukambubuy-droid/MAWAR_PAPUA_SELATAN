...
export function SeasonFilters({ years, year, onYear, seasons, seasonId, onSeason }: {
  ...
  return <div className="season-filter-group"><div className="season-filter-title">PERIODE</div><div className="season-filter-pair">
    ...
    <label className="wide"><span>Musim Tanam</span><select aria-label="Musim Tanam" value={seasonId} onChange={e => onSeason(e.target.value)}><option value="all">Ringkasan Semua Musim {year}</option>{seasons.map(item => <option key={item.id} value={item.id}>{item.name} ({item.status})</option>)}</select></label>
  </div></div>;
}
