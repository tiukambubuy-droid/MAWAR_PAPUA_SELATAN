...
export default function ProductionPage() {
  ...
  const regencyDetail = useMemo<ProductionRecord>(()=>({...total,id:"93.01",name:"Merauke",level:"Kabupaten"}),[total]);
  return <div className="subpage page-enter production-page">
    ...
    {!districtId&&!villageId&&<div className="production-regency-detail"><button className="production-detail-button" aria-label={`Lihat rincian produksi Kabupaten Merauke, ${season?.display_name ?? season?.name ?? seasonId}`} onClick={()=>setDetail(regencyDetail)}>Lihat rincian produksi Kabupaten Merauke</button></div>}
