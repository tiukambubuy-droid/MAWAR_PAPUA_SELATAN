...
import { formatMonitoringDate, formatMonitoringSourceType, formatMonitoringStatus, formatMonitoringTimestamp } from "@/lib/monitoring-presentation";
  ...
  return <div className="monitoring-modal-backdrop" onMouseDown={event => event.target === event.currentTarget && onClose()}>
    <div ref={panel} className="monitoring-modal" role="dialog" aria-modal="true" aria-labelledby="food-detail-title" tabIndex={-1}>
      ...
      <div className="monitoring-modal-body">
        ...
        <h3>Periode, sumber, dan validasi</h3><dl className="detail-metrics"><div><dt>Musim</dt><dd>{item.seasonLabel}</dd></div><div><dt>Cut-off</dt><dd>{formatMonitoringDate(item.cutoff)}</dd></div><div><dt>Status monitoring</dt><dd>Dipantau</dd></div><div><dt>Status validasi</dt><dd>{validationLabel(item.validationStatus)}</dd></div><div><dt>Jenis sumber</dt><dd>{formatMonitoringSourceType("prototype")}</dd></div><div><dt>Jenis data</dt><dd>{formatMonitoringStatus("simulation")}</dd></div><div><dt>Diperbarui</dt><dd>{formatMonitoringTimestamp(item.updatedAt)}</dd></div></dl><p>Sumber: {item.sourceReference}</p>
